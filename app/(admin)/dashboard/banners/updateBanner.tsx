...
function UpdateBanner({ bannerId }: { bannerId: string }) {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          `https://travel-journal-api-bootcamp.do.dibimbing.id/api/v1/get-banner/${bannerId}`,
          {
            method: "GET",
            headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error("Failed to fetch banner: " + errorData.message);
        }
        ...
        Swal.fire("Failed to fetch banner: " + error);
      } finally {
        setIsLoading(false);
      }
    };
